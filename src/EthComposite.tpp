...
void BlockTransaction::RLPparse(ByteSet<BYTE> &b)
{
    switch(getType()) {
        case 1: //EIP-2930 (11 fields)
            ...
            for(uint i = 0; i < 7; i++) create<ByteSetField>(b);
        ...
        case 2: //EIP-1559 (12 fields)
            ...
            create<BlockAccessLists>(b);
            for(uint i = 9; i < 12; i++) create<ByteSetField>(b);
            break;
        case 3: //EIP-4844 (14 fields)
            //chain_id, nonce, max_priority_fee_per_gas, max_fee_per_gas, gas_limit, to, value, data, access_list, max_fee_per_blob_gas, blob_versioned_hashes, y_parity, r, s
            for(uint i = 0; i < 8; i++) create<ByteSetField>(b);
            create<BlockAccessLists>(b);
            create<ByteSetField>(b);
            create<BlockBlobVersionHashes>(b);
            for(uint i = 11; i < 14; i++) create<ByteSetField>(b);
            break;
